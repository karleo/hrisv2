...
final class HtmlParser
{
    ...
    public function parseWithDiagnostics(string $html, string $timezone): ParseResult
    {
    ...
    /**
     * @return array{punches: list<PunchRecord>, rows_scanned: int, data_rows: int, layout: ?string}
     */
     ...
     * @return array{date_index: int, id_index: int, time_columns: array<int, PunchDirection>}|null
     */
    private function detectGridLayout(array $headers): ?array
    {
        $dateIndex = null;
        $idIndex = null;
        /** @var array<int, PunchDirection> $timeColumns */
        $timeColumns = [];
        ...
        foreach ($headers as $index => $header) {
            ...
            if ($normalized === 'in' || str_starts_with($normalized, 'in ')) {
                $timeColumns[$index] = PunchDirection::In;

                continue;
            }

            if ($normalized === 'out' || str_starts_with($normalized, 'out ')) {
                $timeColumns[$index] = PunchDirection::Out;
            }
        }
    ...
    /**
     * @param  array{date_index: int, id_index: int, time_columns: array<int, PunchDirection>}  $layout
     * @return list<PunchRecord>
     */
    private function parseGridLayout(\DOMNodeList $rows, array $layout, string $timezone): array
    {
        ...
        for ($i = 1; $i < $rows->length; $i++) {
            ...
            foreach ($layout['time_columns'] as $columnIndex => $direction) {
                ...
                $punches[] = PunchRecord::fromDeviceWallClock(
                    deviceUserId: $deviceUserId,
                    punchedAtStorage: PunchClock::normalizeWallClock($date, $time),
                    direction: $direction,
                    timezone: $timezone,
                    rawPayload: [
                    ...
                    rawStatus: $direction === PunchDirection::In ? 0 : 1,
                );
            }
        }
    ...
    private function directionFromState(string $state, string $timestampFallback): PunchDirection
    {
    ...
    {
        $normalized = PunchClock::normalizeTimestamp($timestampRaw);

        if (preg_match('/^\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}$/', $normalized) === 1) {
            return $normalized;
        }

        return PunchClock::normalizeWallClock(
